...
class PoliceAlreadyExistsException(ConflictException):
    def __init__(self):
        super().__init__("Police credentials already exist. Only one police account is allowed.")
...
class PoliceService:
    ...
    async def verify_police_credentials(self, email: str, password: str) -> PoliceEntity:
        ...
        if verify_password(password, police.hashed_password):
            return police
        raise CredentialsException()
